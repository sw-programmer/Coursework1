...
[data_train, data_test] = getData('Toy_Circle');

disp(data_train)
disp(size(data_train))
disp(class(data_train))
%%%%%%%%%%%%%
...
save('data.mat', 'data_train', 'data_test')


%% Load Data // 여기서부터 우리 코드 적용된거야!!
clear all
...
for i=1:10
    for j=1:15
        data_train{i,j} = cat(2,data_train{i,j},i);
        data_train_fin = cat(1,data_train_fin,data_train{i,j});
    end
end

for i=1:10
    for j=1:15
        data_test{i,j} = cat(2,data_test{i,j},i);
        data_test_fin = cat(1,data_test_fin,data_test{i,j});
    end
end

data_train = data_train_fin
data_test = data_test_fin
...
test_point = data_test(1:4,:);
figure(1)
%plot_toydata(data_train);
%plot(test_point(:,1), test_point(:,2), 's', 'MarkerSize',20, 'MarkerFaceColor', [.9 .9 .9], 'MarkerEdgeColor','k');

for n=1:4 % 4개의 test datd에 대한 것
    %figure(2)
    %subplot(1,2,1)
    %plot_toydata(data_train);
    %subplot(1,2,2)
...
y_pred = []
for i=1:150
    [~,argmax] = max(p_rf_all(i,:));
    y_pred = cat(2, y_pred, argmax);
end

disp(y_pred)
y_truth = reshape(data_train(:,257),[1,150])
disp(y_truth)
%% Confusion matrix

accuracy = sum(y_truth == y_pred,'all')/numel(y_pred)
disp(accuracy)
fig = figure;
cm = confusionchart(y_truth,y_pred,'RowSummary','row-normalized','ColumnSummary','column-normalized');
cm.Title = accuracy;
...
for N = [2,5,10,15] % Number of trees, try {1,3,5,10, or 20}
    param.num = 50;
    param.depth = N;    % trees depth
    param.splitNum = 10; % Number of trials in split function
    param.split = 'IG'; % Currently support 'information gain' only
    ...
    y_truth = reshape(data_test(:,257),[1,150])
    
    
    accuracy = sum(y_truth == y_pred,'all')/numel(y_pred)
    accu_list(end+1) = accuracy
...
for N = [5,10,30,50] % Number of trees, try {1,3,5,10, or 20}
    param.num = 50;
    param.depth = 5;    % trees depth
    param.splitNum = N; % Number of trials in split function
    param.split = 'IG'; % Currently support 'information gain' only
...
X = [1,2,3,4]
plot(X,accu_list)
title('Accuracy according to Depth')
xlabel('Depth : 2, 5, 10, 15')
ylabel('Accuracy')
%% 
